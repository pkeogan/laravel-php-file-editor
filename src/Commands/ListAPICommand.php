...
class ListAPICommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Pkeogan:api {--provider=} {--group}';
    ...
    protected function showAtoZ()
    {
        ...
        $formattedAPI = $this->api()->map(function ($endpoints, $provider) {
            return collect($endpoints)->map(function ($endpoint) use ($endpoints, $provider) {
                return [
                    $endpoint,
                    Str::replaceFirst('Pkeogan\\Endpoints\\', '', $provider),
                ];
            })->toArray();
    ...
    {
        return (new \Pkeogan\LaravelFile)
    ...
    {
        LaravelFile::in('packages/ajthinking/Pkeogan/src/Endpoints')
        ->get()
        ->map->getReflection()
        ->map->getMethods()
        ->flatten()
