...
class DocumentationCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Pkeogan:docs';
    ...
    {
        $endpoints = PHPFile::in('packages/ajthinking/Pkeogan/src/Endpoints')->get()
        ...
        File::put(
            base_path('packages/ajthinking/Pkeogan/docs/api.md'),
            $content
        );
    }
    ...
    {
        return Str::of(base_path('packages/ajthinking/Pkeogan/docs/src/'))
            ->append(
                Str::of($class)
                    ->replaceFirst('Pkeogan\\', '')
                    ->replace('\\', '/')
                    ->finish('.md')
            );
    }
    ...
    {
        $sourceLink = 'https://github.com/ajthinking/Pkeogan/blob/master/src/'
            . Str::of($class)
            ->replaceFirst('Pkeogan\\', '')
            ->replace('\\', '/')
            ->finish('.php');
    ...
    protected function getEndpointExamples($class)
    {
        ...
        $extractor = new \Pkeogan\Support\DocumentationExtractor(
            $class,
            ['example', 'source']
        );
